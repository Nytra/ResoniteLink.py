from resonitelink.json import json_model, json_element, json_list
...
class ImportMeshRawData(BinaryPayloadMessage):
    ...
    vertex_count : int = json_element("vertexCount", int)
    
    # Do vertices have normals?
    has_normals : bool = json_element("hasNormals", bool)
    
    # Do vertices have tangents?
    has_tangents : bool = json_element("hasTangents", bool)
    
    # Do vertices have colors?
    has_colors : bool = json_element("hasColors", bool)
    ...
    bone_weight_count : int = json_element("boneWeightCount", int)
    ...
    uv_channel_dimensions : List[int] = json_list("uvChannelDimensions", int)

    # Submeshes that form this mesh. Meshes will typically have at least one submesh.
    submeshes : List[SubmeshRawData] = json_list("submeshes", SubmeshRawData)
    ...
    blendshapes : List[BlendshapeRawData] = json_list("blendshapes", BlendshapeRawData)
